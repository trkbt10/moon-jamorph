...
const tinyTokenizer = await createTokenizer({
  profile: "tiny",
  compressed: true,
});
const tinyTokens = tinyTokenizer.tokenize(sentence);
console.log(`dic-tiny=${tinyTokens.map((t) => t.surface).join("|")}`);
console.log(`tiny entries=${tinyTokenizer.stats.entryCount}`);

const miniTokenizer = await createTokenizer({
  profile: "mini",
  compressed: true,
});
const miniTokens = miniTokenizer.tokenize(sentence);
console.log(`dic-mini=${miniTokens.map((t) => t.surface).join("|")}`);
console.log(`mini entries=${miniTokenizer.stats.entryCount}`);

if (tinyTokens.length === 0) {
  throw new Error("tiny tokenizer returned no tokens");
}
if (miniTokens.length === 0) {
  throw new Error("mini tokenizer returned no tokens");
}
console.log("profile=tiny-ok");
console.log("profile=mini-ok");
NODE
node --input-type=module <<'NODE'
