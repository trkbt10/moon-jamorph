...
const wasm = await createMicadoWasm({ nanoProfile: "tiny", miniProfile: "mini", compressed: true });
...
if (JSON.stringify(wasmNano) !== JSON.stringify(dicCompact)) {
  throw new Error("createMicadoWasm(tokenizeNano) must match createTokenizer(profile=tiny)");
}
...
if (JSON.stringify(wasmMini) !== JSON.stringify(miniCompact)) {
  throw new Error("createMicadoWasm(tokenizeMini) must match createTokenizer(profile=mini)");
}
